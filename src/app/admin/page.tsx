              ...
              <div style={{ display: 'flex', gap: '2rem', marginBottom: '2rem' }}>
                <div style={{ flex: '0 0 300px', backgroundColor: 'rgba(214, 180, 124, 0.05)', borderRadius: '1rem', border: '1px solid rgba(214, 180, 124, 0.2)', overflow: 'hidden' }}>
                  <div style={{ padding: '1rem', backgroundColor: 'rgba(0,0,0,0.4)', color: 'var(--primary)', fontWeight: 'bold', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <select 
                      value={selectedOrder.slipStatus}
                      onChange={(e) => {
                        setSelectedOrder({...selectedOrder, slipStatus: e.target.value});
                        updateOrderStatusLocal(selectedOrder.dbId, e.target.value, selectedOrder.serviceStage);
                      }}
                      style={{ 
                    ...
                    >
                      ...
                      <option value="approved">ถูกต้อง</option>
                      <option value="rejected">ไม่ถูกต้อง</option>
                    </select>
                  </div>
                ...
                <div>
                  ...
                  <div style={{ backgroundColor: 'rgba(214, 180, 124, 0.05)', padding: '1.5rem', borderRadius: '1rem', border: '1px solid rgba(214, 180, 124, 0.2)', marginBottom: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                      ...
                      {(selectedOrder.customerInfo?.birthdate || selectedOrder.customerInfo?.dob) && <div><strong style={{ color: 'var(--text-muted)' }}>วันเกิด:</strong> <span style={{ color: 'var(--text-main)' }}>{selectedOrder.customerInfo?.birthdate || selectedOrder.customerInfo?.dob}</span></div>}
                      {(selectedOrder.customerInfo?.birthtime || selectedOrder.customerInfo?.birthTime) && <div><strong style={{ color: 'var(--text-muted)' }}>เวลาตกฟาก:</strong> <span style={{ color: 'var(--text-main)' }}>{selectedOrder.customerInfo?.birthtime || selectedOrder.customerInfo?.birthTime}</span></div>}
                      {(selectedOrder.customerInfo?.birthprovince || selectedOrder.customerInfo?.birthProvince) && <div><strong style={{ color: 'var(--text-muted)' }}>จังหวัดที่เกิด:</strong> <span style={{ color: 'var(--text-main)' }}>{selectedOrder.customerInfo?.birthprovince || selectedOrder.customerInfo?.birthProvince}</span></div>}
                      ...
                      {(selectedOrder.customerInfo?.job || selectedOrder.customerInfo?.career) && <div><strong style={{ color: 'var(--text-muted)' }}>อาชีพ:</strong> <span style={{ color: 'var(--text-main)' }}>{selectedOrder.customerInfo?.job || selectedOrder.customerInfo?.career}</span></div>}
                      {(selectedOrder.customerInfo?.relationship || selectedOrder.customerInfo?.status) && <div><strong style={{ color: 'var(--text-muted)' }}>สถานภาพ:</strong> <span style={{ color: 'var(--text-main)' }}>{selectedOrder.customerInfo?.relationship || selectedOrder.customerInfo?.status}</span></div>}
                      {(selectedOrder.customerInfo?.focus || selectedOrder.customerInfo?.extraNote) && <div><strong style={{ color: 'var(--text-muted)' }}>เรื่องที่อยากเน้นย้ำ:</strong> <span style={{ color: 'var(--primary)' }}>{selectedOrder.customerInfo?.focus || selectedOrder.customerInfo?.extraNote}</span></div>}
                    </div>
