            ...
            {order.customerInfo && (
              <div style={{ backgroundColor: 'rgba(214, 180, 124, 0.05)', padding: '1.5rem', borderRadius: '1rem', border: '1px solid rgba(214, 180, 124, 0.2)', marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  ...
                  {(order.customerInfo?.birthdate || order.customerInfo?.dob) && <div><strong style={{ color: 'var(--text-muted)' }}>วันเกิด:</strong> <span style={{ color: 'var(--text-main)' }}>{order.customerInfo?.birthdate || order.customerInfo?.dob}</span></div>}
                  {(order.customerInfo?.birthtime || order.customerInfo?.birthTime) && <div><strong style={{ color: 'var(--text-muted)' }}>เวลาตกฟาก:</strong> <span style={{ color: 'var(--text-main)' }}>{order.customerInfo?.birthtime || order.customerInfo?.birthTime}</span></div>}
                  {(order.customerInfo?.birthprovince || order.customerInfo?.birthProvince) && <div><strong style={{ color: 'var(--text-muted)' }}>จังหวัดที่เกิด:</strong> <span style={{ color: 'var(--text-main)' }}>{order.customerInfo?.birthprovince || order.customerInfo?.birthProvince}</span></div>}
                  ...
                  {(order.customerInfo?.job || order.customerInfo?.career) && <div><strong style={{ color: 'var(--text-muted)' }}>อาชีพ:</strong> <span style={{ color: 'var(--text-main)' }}>{order.customerInfo?.job || order.customerInfo?.career}</span></div>}
                  {(order.customerInfo?.relationship || order.customerInfo?.status) && <div><strong style={{ color: 'var(--text-muted)' }}>สถานภาพ:</strong> <span style={{ color: 'var(--text-main)' }}>{order.customerInfo?.relationship || order.customerInfo?.status}</span></div>}
                  {(order.customerInfo?.focus || order.customerInfo?.extraNote) && <div><strong style={{ color: 'var(--text-muted)' }}>เรื่องที่อยากเน้นย้ำ:</strong> <span style={{ color: 'var(--primary)' }}>{order.customerInfo?.focus || order.customerInfo?.extraNote}</span></div>}
                </div>
